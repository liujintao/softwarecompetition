...
<html>
 ...
 <% User auser=(User)session.getAttribute("user");
  ...
  String deviceId=mySmartUpload.getRequest().getParameter("deviceInf");
	...
	int fileSize=0;        //文件大小
	String url="teamupload/";      //应保证在根目录中有此目录的存在
	String fileType ="";
...
//String teamid=mySmartUpload.getRequest().getParameter("teamid");
  //  String name=mySmartUpload.getRequest().getParameter("name");
	...
	// 循环取得上传所有文件
	  ...
	  if(i<=0){
	   ...
	   stmt.executeUpdate("update team set put='Y',articlename='"+filename+"' where teamid='"+teamid+"';");
	    }
	  else{
